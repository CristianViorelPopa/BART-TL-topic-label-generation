...
def main():
    ...
    topics = [topic_terms_separator.join(terms)
              for terms in [[term['word'] for term in topic[:max_terms_per_topic]]
                            for topic in topics]]
    ...
    # build the dataset by joining topic with their labels (one sample for each different label)
    dataset = np.array([[[topics[idx], topic_label] for topic_label in labels[idx]]
                       for idx in range(len(topics))])

    new_dataset = []
    for topic in dataset:
        new_dataset.extend(topic)
