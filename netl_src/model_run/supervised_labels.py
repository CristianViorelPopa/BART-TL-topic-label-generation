...
def prepare_features(letter_tg_dict,page_rank_dict,cols,feature_names):
    ...
    for x in range(0,len(letter_tg_dict)):
        ...
        for k in a:
            ...
            # TODO: use the probs here
            topic_words = [topic_term['word'] for topic_term in topic_list[x]]
            com_words = set(word_labels).intersection(set(topic_words)) # Extracting topic overlap.
            if prob_based_ranking:
                com_word_prob = sum([float(term['prob']) for term in topic_list[x] if term['word'] in com_words])
            else:
                com_word_prob = len(com_words)
            lab_length = len(word_labels) # number of words in the candidate label.   
            new_list.append(lab_length)
            new_list.append(com_word_prob)
...
def get_predictions(test_set,num):
    ...
    query3 ="rm test_temp.dat predictionstemp"   # deleting the test file and prediction file generated as part of code to run svm_classify
    os.system(query3)
